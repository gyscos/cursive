    ...
    fn parse_mouse_event(&mut self) -> Event {
        ...
        if ncurses::getmouse(&mut mevent as *mut ncurses::MEVENT) == ncurses::OK {
            ...
            if mevent.bstate == ncurses::REPORT_MOUSE_POSITION as mmask_t {
                ...
                // a weird double-release event, or (on ncurses builds that
                // don't support a 5th mouse button, e.g. the legacy
                // ncurses 5.x that macOS ships as its system library) a
                // wheel-down scroll. See `disambiguate_position_report` for
                // details.
