...
impl View for LinearLayout {
    ...
    fn required_size(&mut self, req: Vec2) -> Vec2 {
        ...
        // we're gonna use this a lot for a bit.
        let o = self.orientation;

        // split the screen evenly...
        let available_size = req.get(o);
        let even_size = available_size / self.children.len();
        ...
        let even_min: Vec<Vec2> = ideal_sizes
            .iter()
            .map(|i| {
        ...
        debug!("available size {} used size {}", available_size, used_size);

        // only do this if there is some to give away.
        let good_fit = if used_size < available_size {
            // get a count of how many want more than the even amount
            let big_cnt = even_min
                .iter()
                .zip(ideal_sizes.iter())
                .filter(|(even, ideal)| ideal.get(o) > even.get(o))
                .count();

            let extra_size = available_size - used_size; // unsigned math ok because of if above.
            ...
            if big_cnt > 0 {
                ...
                // give it away...
                let mut adj_even: Vec<Vec2> = even_min
                    .iter()
                    .zip(ideal_sizes.iter())
                    .map(|(even, ideal)| {
                ...
                if odd_bit > 0 {
                    ...
                    let bigest = adj_even
                        .iter_mut()
                        .reduce(|a, b| if a.get(o) > b.get(o) { a } else { b })
                        .unwrap();
            ...
            // all the children are huge. no one wants less than the max.
        ...
        self.children.iter_mut().zip(good_fit.iter()).for_each(
            |(child, fit)| {
